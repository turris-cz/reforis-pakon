...
install:
	...
	ln -sf /tmp/reforis-pakon/reforis_static/reforis_pakon /tmp/reforis/reforis_static/
...
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m pylint --rcfile=pylintrc reforis_pakon
	$(VENV_BIN)/$(PYTHON) -m pycodestyle --config=pycodestyle reforis_pakon
...
create-messages:
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_pakon/translations/messages.pot .
init-langs: create-messages
	for lang in $(LANGS); do \
		$(VENV_BIN)/pybabel init \
		-i reforis_pakon/translations/messages.pot \
		-d reforis_pakon/translations/ -l $$lang \
	; done
update-messages:
	$(VENV_BIN)/pybabel update -i ./reforis_pakon/translations/messages.pot -d ./reforis_pakon/translations
compile-messages:
	$(VENV_BIN)/pybabel compile -f -d ./reforis_pakon/translations

clean:
	find . -name '*.pyc' -exec rm -f {} +
	rm -rf $(VENV_NAME) *.eggs *.egg-info dist build .cache
	rm -rf dist build *.egg-info
	rm -rf $(JS_DIR)/node_modules/ reforis_static/reforis_pakon/js/app.min.js
	$(PYTHON) -m pip uninstall -y reforis_pakon
